...
-- 7. 店舗画像のポリシー
CREATE POLICY "Anyone can view active store images" ON store_images FOR SELECT USING (is_active = true);
CREATE POLICY "Store owners can manage their store images" ON store_images FOR ALL USING (true);

-- 8. 店舗掲示板のポリシー
CREATE POLICY "Anyone can view active store bulletins" ON store_bulletins FOR SELECT USING (is_active = true);
CREATE POLICY "Store owners can manage their store bulletins" ON store_bulletins FOR ALL USING (true);
...
CREATE POLICY "Store owners can manage their store tag relations" ON store_tag_relations FOR ALL USING (true);
