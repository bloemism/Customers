...
-- 1. お気に入り店舗テーブル（stores.idがtext型の場合）
CREATE TABLE IF NOT EXISTS favorite_stores (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id UUID NOT NULL,
  store_id TEXT NOT NULL,  -- stores.idがtext型の場合
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(customer_id, store_id)
);

-- 2. レッスン予約テーブル（lessons.idがtext型の場合）
CREATE TABLE IF NOT EXISTS lesson_bookings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id UUID NOT NULL,
  lesson_id TEXT NOT NULL,  -- lessons.idがtext型の場合
...
CREATE TABLE IF NOT EXISTS customer_point_history (
  ...
  store_id TEXT,  -- stores.idがtext型の場合
  transaction_id UUID,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- 6. 外部キー制約の追加（型に合わせて修正）
-- customersテーブルが存在する場合のみ
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'customers') THEN
    ALTER TABLE favorite_stores 
    ADD CONSTRAINT fk_favorite_stores_customer 
    FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
    
    ALTER TABLE lesson_bookings 
    ADD CONSTRAINT fk_lesson_bookings_customer 
    FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
    
    ALTER TABLE customer_point_history 
    ADD CONSTRAINT fk_point_history_customer 
    FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE CASCADE;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'stores') THEN
    ALTER TABLE favorite_stores 
    ADD CONSTRAINT fk_favorite_stores_store 
    FOREIGN KEY (store_id) REFERENCES stores(id) ON DELETE CASCADE;
    
    ALTER TABLE customer_point_history 
    ADD CONSTRAINT fk_point_history_store 
