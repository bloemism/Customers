...
-- payment_codesテーブルが存在する場合のみ外部キー制約を削除
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payment_codes' AND table_schema = 'public') THEN
        ALTER TABLE payment_codes DROP CONSTRAINT IF EXISTS payment_codes_store_id_fkey;
        DROP TABLE payment_codes CASCADE;
        RAISE NOTICE '既存のpayment_codesテーブルを削除しました';
    ELSE
        RAISE NOTICE 'payment_codesテーブルは存在しません';
    END IF;
END $$;
